...
import (
	...
	dmspec "github.com/pingcap/tiup/components/tem/spec"

	"github.com/pingcap/tiup/pkg/cluster/spec"
	"github.com/pingcap/tiup/pkg/set"
)

// UpdateTEMMeta is used to maintain the DM meta information
type UpdateTEMMeta struct {
	cluster        string
	metadata       *dmspec.Metadata
	deletedNodesID []string
}

// NewUpdateTEMMeta create i update tem meta task.
func NewUpdateTEMMeta(cluster string, metadata *dmspec.Metadata, deletedNodesID []string) *UpdateTEMMeta {
	return &UpdateTEMMeta{
		cluster:        cluster,
		metadata:       metadata,
...
func (u *UpdateTEMMeta) Execute(ctx context.Context) error {
	deleted := set.NewStringSet(u.deletedNodesID...)
	topo := u.metadata.Topology
	workers := make([]*dmspec.WorkerSpec, 0)
	for i, instance := range (&dmspec.TEMWorkerComponent{Topology: topo}).Instances() {
...
func (u *UpdateTEMMeta) Rollback(ctx context.Context) error {
	return dmspec.GetSpecManager().SaveMeta(u.cluster, u.metadata)
}

// String implements the fmt.Stringer interface
func (u *UpdateTEMMeta) String() string {
