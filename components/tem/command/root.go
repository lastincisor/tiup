...
func init() {
	...
	rootCmd = &cobra.Command{
		Use:   tui.OsArgs0(),
		Short: "(EXPERIMENTAL) Deploy a TEM cluster",
...
please backup your data before process.`,
		...
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			...
			if err = cspec.Initialize("tem"); err != nil {
				return err
			}

			temspec = spec.GetSpecManager()
			logger.EnableAuditLog(cspec.AuditDir())
			cm = manager.NewManager("tem", temspec, spec.DMComponentVersion, log)
	...
	if err != nil {
		...
		default:
			...
			if !errorx.HasTrait(err, utils.ErrTraitPreCheck) {
				logger.OutputDebugLog("tiup-tem")
			}
