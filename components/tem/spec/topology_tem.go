...
var (
	...
	serverConfigsTypeName = reflect.TypeOf(TEMServerConfigs{}).Name()
)
...
type (
	// TEMServerConfigs represents the server runtime configuration
	TEMServerConfigs struct {
	...
	Specification struct {
		...
		ServerConfigs    TEMServerConfigs         `yaml:"server_configs,omitempty" validate:"server_configs:ignore"`
...
func (s *Specification) UnmarshalYAML(unmarshal func(any) error) error {
	...
	if err := fillTEMCustomDefaults(&s.GlobalOptions, s); err != nil {
		return err
	}
...
func (s *Specification) MergeTopo(rhs spec.Topology) spec.Topology {
	...
	if !ok {
		panic("topo should be TEM Topology")
	}
...
func fillTEMCustomDefaults(globalOptions *GlobalOptions, data any) error {
	v := reflect.ValueOf(data).Elem()
	t := v.Type()

	var err error
	for i := 0; i < t.NumField(); i++ {
		if err = setTEMCustomDefaults(globalOptions, v.Field(i)); err != nil {
			return err
		}
	}
...
func setTEMCustomDefaults(globalOptions *GlobalOptions, field reflect.Value) error {
	...
	case reflect.Slice:
		for i := 0; i < field.Len(); i++ {
			if err := setTEMCustomDefaults(globalOptions, field.Index(i)); err != nil {
				return err
			}
		}
	case reflect.Struct:
		...
		ref.Elem().Set(field)
		if err := fillTEMCustomDefaults(globalOptions, ref.Interface()); err != nil {
			return err
		}
		field.Set(ref.Elem())
	case reflect.Ptr:
		if err := fillTEMCustomDefaults(globalOptions, field.Elem()); err != nil {
			return err
		}
	}
